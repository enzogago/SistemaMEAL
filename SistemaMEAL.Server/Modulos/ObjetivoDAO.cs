    ...
    public class ObjetivoDAO
    {
         ...
         public (string? anoOut,string? codOut,string? message, string? messageType) Insertar(ClaimsIdentity? identity, Objetivo objetivo)
        {
            ...
            try
            {
                ...
                // Deserializa la cadena JSON en una lista de objetos Estado
                List<SubProyecto>? temporal = JsonConvert.DeserializeObject<List<SubProyecto>>(jsonResult.ToString());
                SubProyecto consulta = temporal[0];

                var accPad = "PROYECTO-" + consulta.ProAno + "-" + consulta.ProCod + "-SUB_PROYECTO-" + consulta.SubProAno + "-" + consulta.SubProCod + "-OBJETIVO-" + anoOut + "-" + codOut;
